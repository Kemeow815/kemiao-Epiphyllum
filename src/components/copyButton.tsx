...
export default function CopyButton() {
    const handleCopy = (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const preBlock = Btn.closest(".code-block")?.querySelector("pre");
    ...
    return (
        <button
            className="copy-btn btn-regular-dark absolute active:scale-90 h-8 w-8 top-2 right-2 opacity-75 text-sm p-1.5 rounded-lg transition-all ease-in-out"
            onClick={handleCopy}
        >
            <div>
                <svg
